...
server <- function(input, output, session) {
  ...
  terminate_bg_process <- function(proc) {
    if (is.null(proc)) return(FALSE)

    is_alive <- tryCatch(isTRUE(proc$is_alive()), error = function(e) FALSE)
  ...
  observeEvent(input$gen_keyfiles, {
    ...
    if (is.null(job_queue) || nrow(job_queue) == 0) {
      ...
      shinyjs::hide("kill_gen_btn_wrap")
      step_start_gen(NULL)
    ...
    shinyjs::show("kill_gen_btn_wrap")
